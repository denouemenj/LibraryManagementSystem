			...
			<tr>
				...
				<td>
					<button type="button" class="btn btn-default disabled" disabled>
						<c:if test="${backBook.backStatus==0}">Not Returned</c:if>
						<c:if test="${backBook.backStatus==1}">Await</c:if>
						<c:if test="${backBook.backStatus==2}">Returned</c:if>
					</button>
				</td>
				<td>
					<c:if test="${backBook.backStatus==1}">
						<a type="button" class="btn btn-success" href="<%=request.getContextPath()%>/admin/allowBackBook?id=${backBook.id}">Return Confirmed</a>
					</c:if>	
				</td>
			</tr>
		</c:forEach>
